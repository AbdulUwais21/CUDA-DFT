...
int main(int argc, char **argv) // int argc, char **argv
{
    ...
    cplx *y_d = (cplx *)malloc(sizeof(cplx)*signalSize*2);
    memset(y_d, 0, sizeof(cplx)*signalSize*2);
    ...
    cplx *d_y;
    checkCudaErrors(cudaMalloc((void **)&d_y, sizeof(cplx)*signalSize*2));
    checkCudaErrors(cudaMemset(d_y, 0, sizeof(cplx)*signalSize*2));
    // cplx *d_temp;
    ...
    //CUDA Memcpy
    checkCudaErrors(cudaMemcpy(y_d, d_y, sizeof(cplx)*signalSize*2, cudaMemcpyDeviceToHost));
...
__global__ void dft_CUDA( cplx *in,  cplx *out, int n)
{
    ...
    // {
        for (int i = 0; i < n ; i++ )
        {
            out[idx+n].x = (in[i].x *cos(phaseDFT(idx,i,n)) + in[i].y * sin(phaseDFT(idx, i, n)));
            __syncthreads();
            out[idx+n].y = ((-1) * in[i].x * sin(phaseDFT(idx, i, n)) + in[i].y * cos(phaseDFT(idx, i, n)));
            __syncthreads();
