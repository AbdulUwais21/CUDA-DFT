...
__device__ void sumDFT_CUDA(cplx *in, cplx *out, int n);
...
int main(int argc, char **argv) // int argc, char **argv
{
    ...
    if (signalSize <= maxThreads)
    {
     ...
     int numBlock = 1; //signalSize / n_threads_per_blocks;
    int n_threads_per_blocks2 = signalSize / n_threads_per_blocks;
    //int pKali = 1;

    dim3 block (n_threads_per_blocks, n_threads_per_blocks2);

    printf("Number Threads / Block = %d and %d\n", n_threads_per_blocks, n_threads_per_blocks2);
    ...
    checkCudaErrors(cudaMemset(d_y, 0, sizeof(cplx)*signalSize*signalSize));
    // cplx *d_temp;
    ...
    dft_CUDA<<<numBlock, block, sizeof(cplx)*signalSize*2>>>(d_x, d_y, signalSize);
    // cudaThreadSynchronize();
    // checkCudaErrors(cudaThreadSynchronize());
    // sumDFT_CUDA<<<1, 1>>>(d_y, d_x, signalSize);
    // cudaThreadSynchronize();
    ...
    printf("Size Shared Temp = %d\n", sizeof(cplx)*signalSize*signalSize);
    /////////////////CPU TIMER////////////////////////
    printf("\nCPU Malloc, Memset, and Initializing = %f\n", cpuTimer1);
    printf("CPU DFT Function = %f\n", cpuTimer2);
...
__global__ void dft_CUDA( cplx *in,  cplx *out, int n)
{
    ...
    unsigned int cacheIdx1 = blockDim.x / 2;
    int k = 0, j = 0, l = 0;
    // unsigned int icrIdx = blockDim.x * gridDim.x;
    ...
    if (idx < n)
    {
        ...
        {
            k = 0;
            temp[idx+k*n].x = (float)(in[i].x *cos(phaseDFT(idx,i,n)) + in[i].y * sin(phaseDFT(idx, i, n)));
            temp[idx+k*n].y = (float)((-1) * in[i].x * sin(phaseDFT(idx, i, n)) + in[i].y * cos(phaseDFT(idx, i, n)));
            __syncthreads();
            if ( j == 1)
            {
                out[idx].x = temp[idx].x + temp[idx+cacheIdx1].x;
                out[idx].y = temp[idx].y + temp[idx+cacheIdx1].y;
                __syncthreads();
                l++;
            }
             else if (l == 1) 
             {
                out[idx].x = out[idx].x + temp[idx].x;
                out[idx].y = out[idx].y + temp[idx].y;
                __syncthreads();
             }
            j++;
