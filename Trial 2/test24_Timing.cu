...
{
    unsigned int idx = threadIdx.x + blockIdx.x * blockDim.x;
    float sumr1 = 0.0, sumr2 = 0.0, sumi1 = 0.0, sumi2 = 0.0;
    if (idx < n)
    {
        for (int i = 0; i < n ; i++ )
        {
            sumr1 += (float)(in[i].x *cos(phaseDFT(idx,i,n))); 
            sumr2 += (float)(in[i].y * sin(phaseDFT(idx, i, n)));
            sumi1 += (float)(((-1) * in[i].x * sin(phaseDFT(idx, i, n)))); 
            sumi2 += (float)(in[i].y * cos(phaseDFT(idx, i, n)));
        } 
            out[idx].x = sumr1+sumr2;
            out[idx].y = sumi1+sumi2;
    }
}
