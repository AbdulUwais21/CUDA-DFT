...
int main(int argc, char **argv) // int argc, char **argv
{
    ...
    //CUDA Malloc
    cplx *d_x;
    checkCudaErrors(cudaMalloc((void **)&d_x, sizeof(cplx)*signalSize*signalSize));
    checkCudaErrors(cudaMemset(d_x, 1, sizeof(cplx)*signalSize*signalSize));
    cplx *d_y;
    ...
    sumDFT_CUDA<<<numBlock, n_threads_per_blocks, sizeof(cplx)*signalSize>>>(d_x, d_y, signalSize);
    cudaThreadSynchronize();
