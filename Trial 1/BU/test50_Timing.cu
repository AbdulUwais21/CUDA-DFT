...
int main(int argc, char **argv) // int argc, char **argv
{
    ...
    dft_CUDA<<<numBlock, block, sizeof(cplx)*signalSize*signalSize>>>(d_x, d_y, signalSize);
...
__global__ void dft_CUDA( cplx *in,  cplx *out, int n)
{
    ...
    if (idx < n)
    {
        for (int i = 0; i < n ; i++ )
        {
            temp[idx+i*n].x = (float)(in[i].x *cos(phaseDFT(idx,i,n)) + in[i].y * sin(phaseDFT(idx, i, n)));
            temp[idx+i*n].y = (float)((-1) * in[i].x * sin(phaseDFT(idx, i, n)) + in[i].y * cos(phaseDFT(idx, i, n)));
            __syncthreads();
