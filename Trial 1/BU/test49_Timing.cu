...
int main(int argc, char **argv) // int argc, char **argv
{
    ...
    dft_CUDA<<<numBlock, block>>>(d_x, d_y, signalSize);
...
__global__ void dft_CUDA( cplx *in,  cplx *out, int n)
{
    ...
    if (idx < n)
    {
        for (int i = 0; i < n ; i++ )
        {
            out[idx+i*n].x = (float)(in[i].x *cos(phaseDFT(idx,i,n)) + in[i].y * sin(phaseDFT(idx, i, n)));
            out[idx+i*n].y = (float)((-1) * in[i].x * sin(phaseDFT(idx, i, n)) + in[i].y * cos(phaseDFT(idx, i, n)));
            //printf("out[%u+%d*%d = %d] = %.2f +%.2fi\n",idx, i, n, (idx+i*n), out[idx+i*n].x, out[idx+i*n].y);
            // printf("Phase = %.2f", phaseDFT(idx,i,n));
        }    
    }

    sumDFT_CUDA(out, in, n);
}
