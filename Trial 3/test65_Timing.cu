...
int main(int argc, char **argv) // int argc, char **argv
{
    ...
    grid.x = (elStreamByte + block.x -1)/block.x;
    ...
    cplx *d_y;
    checkCudaErrors(cudaMalloc((void **)&d_y, nBytes));
    ...
    for (int i = 0; i < NStreams ; i++)
    {
        int offsetKernel = i * numElStreams;
        dft_CUDA<<<grid, block, 0, streams[i]>>>(&d_x[offsetKernel], &d_y[offsetKernel], numElStreams);    
    }
    ...
    //CUDA Memcpy
    checkCudaErrors(cudaEventRecord(start, 0));
    checkCudaErrors(cudaMemcpy(y_d, d_y, nBytes, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaEventRecord(stop, 0));
    checkCudaErrors(cudaEventSynchronize(stop));
    ...
    printf("Memcpy H2D : \t %f ms (%f GB/s)\n", cuda_memcpyH2D_time, nBytes*1e-6/cuda_memcpyH2D_time);
    printf("Kernel  : \t %f ms (%f GB/s)\n", kernel_time, nBytes*1e-6/kernel_time);
    printf("Memcpy D2H : \t %f ms (%f GB/s)\n", cuda_memcpyD2H_time, nBytes*1e-6/cuda_memcpyH2D_time);
    printf("GPU Total Time: \t %f ms (%f GB/s)\n", GPU_total_time, nBytes*1e-6/GPU_total_time);
